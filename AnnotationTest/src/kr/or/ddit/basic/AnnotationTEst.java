...
public class AnnotationTEst {
	...
	public static void main(String[] args) {
		...
		Method[] declardedMethods = Service.class.getDeclaredMethods();
		
		for (Method m : declardedMethods) {
			System.out.println(m.getName());	//메서드명 출력
			for (int i = 0; i < m.getDeclaredAnnotation(PrintAnnotation.class).count(); i++) {	//PrintAnnotation의 count값만큼
 ...
 *2. Reflection API는 java.lang.reflection패키지와 java.lang.Class를 통해 제공된다.
 ...
 *6. Reflection API의 기능은 뛰어나지만 약간의 오버헤드가 발생한다. (느린수행속도, 보안취약성, 권한 문제 등)
 *	 그러므로 가급적 마지막 수단으로 사용하는 것으로 고려해야 한다. 	
 * 
 */
