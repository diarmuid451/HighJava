...
public class T14_PhoneBookTest {
	...
	private void insert() {
		...
		System.out.print("주 소 >> ");
		scan.nextLine();  //입력버퍼에 남아 있는 엔터키까지 읽어오는 역할을 수행
						  //next() 호출 후 nextLine()호출시 혹시 남아있을지 모를 쓰레기 값을 처리하기 위해 한번 호출한다.
		String addr = scan.nextLine();
		...
		//이미 등록된 사람인지 검사
		//get()메서드로 값을 가져올대 가져올 자료가 없으면 null을 반환한다.
