...
public class DOMParsing {
	...
	public void parse() {
		try {
			...
			// 속성 접근방법 2: 노드 객체의 getAttribute()메서드 이용(속성 노드)
			NamedNodeMap nodeMap = firstBookNode.getAttributes();
			System.out.println("노드객체의 getAttribute이용 : "+nodeMap.getNamedItem("isbn").getNodeValue());
			
			// 텍스트(노드) 접근
			NodeList bookN1 = firstBookNode.getChildNodes();
			
			// 인덱스 사용시 주의할 것
			// 엔터키에 해당되는 부분이 읽혀질 수 있다.
			...
			System.out.println("titleElement.getTextContent() : "+titleElement.getTextContent());	// 자바초급(title 내용)
			
			// 전체 출력하기
			// 속성값: isbn, kind
			// 엘러먼트 텍스트값 : title, author, price
			System.out.println("------------------------------------------------------------------------");
