...
public class T14_TableViewTest extends Application{
	...
	public void start(Stage primaryStage) throws Exception {
		...
		ObservableList<Member> data =
				...
				new Member("홍길서", "gilseo", 11,"1111-1111", "대후"),		
				new Member("홍길남", "gilnam", 33,"3333-3333", "대상"),		
				new Member("홍길북", "gilbuk", 44,"4444-4444", "대하")
		);		
		...
		engNameCol.setCellValueFactory( 	//셀의 value값을 설정해준다.
				new PropertyValueFactory<>("engName"));  //속성값을 설정
...
//				System.out.println("데이터 오류");
		...
		btnSet.setOnAction(e -> {
			...
			data.set(table.getSelectionModel().getSelectedIndex(),
					...
					Integer.parseInt(txtAge.getText()), txtTel.getText(), txtAddr.getText()));
			infoMsg("작업 결과", txtKorName.getText()+"님 정보를 변경했습니다.");
			txtKorName.clear();
			txtEngName.clear();
			txtAge.clear();
			txtTel.clear();
		...
		btnDel.setOnAction(e -> {
			...
			infoMsg("작업 결과", txtKorName.getText()+"님을 삭제를 완료했습니다.");
			txtKorName.clear();
			txtEngName.clear();
			txtAge.clear();
			txtTel.clear();
		...
		btnTest1.setOnAction(e -> {
			//TextField, TextArea등 문자를 입력하는 객체를 ReadOnly를 실행하는 메서드 : setEditable()
			//이 메서드에 true를 설정하면 '입력가능', false를 설정하면 '읽기 전용'이 된다.
		...
		vbox.getChildren().addAll(btnAdd,btnSet, btnDel, btnTest1, btnTest2);
