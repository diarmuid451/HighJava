 ...
 *  
 *  1. JDBC드라이버 로딩 >> 해당 DB에 접속 >> 질의(SQL명령을 수행한다.) >> 질의 결과를 받아서 처리한다. >> 종료(자원반납)
 *  
 *  1. JDBC드라이버 로딩(Oracle 기준 - JDBC드라이버는 DB를 만든 회사에서 제공한다.) 
 *    	Class.forName("oracle.jdbc.driver.OracleDriver");
 *    
 *  2. 접속하기 : 접속에 성공하면 Connection 객체가 생성된다.
 *  	DriverManager.getConnection()메서드를 이용한다.
 *  
 *  3. 질의 : Statement객체 또는 PreparedStatement객체를 이용하여 SQL문장을 실행한다.
 *  
 *  4. 결과 :
 *  1) SQL문이 select일 경우 >> ResultSet객체가 만들어진다. ResultSet객체에는 select한 결과가 저장된다.
