...
public class Hottel_Map {
	...
	private void roomList(){
		...
		// DB작업에 필요한 객체변수 선언
				...
				try {
					...
					String sql = "select * from hottel"; //실행할 SQL문"
					...
					// 이용하여 차례로 읽어와 처리한다.
					 ...
					 while(rs.next()) {
						 ...
						 System.out.println("방번호 : " + rs.getString("ROOM") + ", 투숙객 : " 
								 						+ rs.getString("NAME"));
					 }
					 System.out.println("------------------------------------------");
	...
	private void checkOut(){
		...
		try {
			...
			String sql = "DELETE FROM HOTTEL WHERE ROOM = (?)";
			...
			String sql1 = "SELECT ROOM FROM HOTTEL WHERE ROOM = "+ room;
